...
<div class="card shadow-sm" style="max-width:440px;">
    <div class="card-body p-0">
        <table class="table table-borderless mb-0">
            <tbody>
                <tr><th width="120">ID</th><td>{{ $item->id }}</td></tr>
                <tr><th>Danh mục</th><td><span class="badge bg-secondary">{{ $item->category?->name }}</span></td></tr>
                <tr><th>Mã</th><td><code>{{ $item->code }}</code></td></tr>
                <tr><th>Tên</th><td><strong>{{ $item->name }}</strong></td></tr>
                <tr><th>Đơn vị</th><td>{{ $item->unit }}</td></tr>
            </tbody>
        </table>
    </div>
</div>
