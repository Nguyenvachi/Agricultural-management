                ...
                <tbody>
                    @forelse ($transactions as $tx)
                        <tr>
                            ...
                            <td
                                class="text-end fw-semibold {{ (float) $tx->quantity_change >= 0 ? 'text-success' : 'text-danger' }}">
                                {{ (float) $tx->quantity_change >= 0 ? '+' : '' }}{{ number_format((float) $tx->quantity_change, 2, '.', ',') }}
                            </td>
                            <td class="text-end text-muted">{{ number_format((float) $tx->balance_before, 2, '.', ',') }}
                            </td>
                            <td class="text-end fw-semibold">{{ number_format((float) $tx->balance_after, 2, '.', ',') }}
                            </td>
                            <td>
                                @if ($tx->order_id)
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="text-center text-muted py-3">Chưa có biến động kho</td>
                        </tr>
                    @endforelse
                </tbody>
                </table>
